...
##				print (currentCase.modWord, currentCase.word)
				currentToken.addCase(case, currentCase)
                                ...
                                if logMax != '?': counters.freqCounter.increment()
                                if romanianGender != '?':
                                        counters.rominfoCounter.increment()
                                        if romanianGender[0] == 'm': counters.Romanian_M.increment()
                                        elif romanianGender[0] == 'f': counters.Romanian_F.increment()
                                        elif romanianGender[0] == 'n':
                                                counters.Romanian_N.increment()
                                                # Split neuter (add to Romanian M and F count later)
                                                if case[-2:] == 'sg': counters.Romanian_NM.increment()
                                                elif case[-2:] == 'pl': counters.Romanian_NF.increment()
                                if slavicGender != '?':
                                        counters.slavinfoCounter.increment()
                                        if slavicGender[0] == 'm': counters.Slavic_M.increment()
                                        elif slavicGender[0] == 'f': counters.Slavic_F.increment()
                                        elif slavicGender[0] == 'n': counters.Slavic_N.increment()
                                if latinGender[0] == 'm':
